...
// 使用 TrackQueueV4 替代 PositionEkf
#include "track_queue_v4.hpp"
#include "math.hpp"
...
/**
 * @brief 装甲板跟踪器类 - 使用TrackQueueV4版本
 * @details 为单个装甲板维护滤波状态，包含装甲板的类型信息
 *          内部统一使用毫米作为长度单位，避免频繁的单位转换
 *          
 * @note 核心改动：
 *       1. 使用TrackQueueV4替代PositionEkf
 *       2. 直接使用解算端的XYZ坐标，避免坐标转换
 *       3. TrackQueueV4的极坐标运动模型更适合曲线运动目标
 */
class ArmorTracker {
public:
    // 单位转换常量 - 统一管理，避免魔数
     ...
     * @param time 观测时间
     * 
     * @note 与原版本不同，这里不再需要噪声参数，
     *       因为TrackQueueV4内部使用固定的噪声配置
     */
...
private:
    ...
    filter_lib::TrackQueueV4 position_filter_;  // V4滤波器（使用极坐标运动模型）
    
    // 初始化相关
    std::deque<Eigen::Vector4d> init_buffer_;  // 初始化缓冲区 [x,y,z,time]
...
/**
 * @brief 目标预测节点 - 使用TrackQueueV4版本
 * @details 订阅PnP解算节点发布的目标数据，进行滤波和预测
 *          
 * @note 主要改动：
 *       1. 现在直接使用解算端的XYZ坐标，不再需要YPD坐标
 *       2. 使用TrackQueueV4的极坐标运动模型
 *       3. 简化了噪声参数配置（V4使用内置参数）
 */
...
private:
     ...
     * @param msg 包含多个目标的消息
     * 
     * @note 现在直接使用消息中的position字段（XYZ坐标）
     */
    ...
    struct Config {
        ...
        int min_init_count = 3;                // V4初始化所需的最小观测点数
