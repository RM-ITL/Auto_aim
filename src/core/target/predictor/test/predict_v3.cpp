...
// ArmorTracker 实现 - 使用TrackQueueV3和XYZ坐标
ArmorTracker::ArmorTracker(int armor_id,
    ...
    // V3需要5维观测数据 [x, y, z, theta, timestamp]
    // 由于没有角度信息，theta设为0
    Eigen::Matrix<double, 5, 1> init_pose;
    init_pose << initial_position_mm.x(), initial_position_mm.y(), initial_position_mm.z(), 0.0, initial_time;
...
void ArmorTracker::update(const Eigen::Vector3d& position_mm, double time) {
    ...
    if (!filter_initialized_) {
        // V3需要5维观测数据，没有角度信息时设为0
        Eigen::Matrix<double, 5, 1> pose;
        pose << position_mm.x(), position_mm.y(), position_mm.z(), 0.0, time;
        ...
        // 尝试初始化V3滤波器（需要至少2个点）
        if (init_buffer_.size() >= 2 && position_filter_.init(init_buffer_)) {
            filter_initialized_ = true;
            RCLCPP_INFO(rclcpp::get_logger("ArmorTracker"),
                       "装甲板 %d V3滤波器初始化成功，使用了 %zu 个观测点",
        ...
        if (dt > 0) {
            // V3支持仅位置观测模式
            Eigen::Matrix<double, 3, 1> position_measurement = position_mm;
            position_filter_.update_position_only(position_measurement, dt);
        }
    }
...
Eigen::Vector3d ArmorTracker::predictPosition(double predict_time) const {
    ...
    // V3状态向量：[x, y, z, theta, vx, vy, vz, omega, ax, ay, b]
    // 提取位置（前3个状态）
...
Eigen::Vector3d ArmorTracker::predictVelocity(double predict_time) const {
    ...
    // V3状态向量：[x, y, z, theta, vx, vy, vz, omega, ax, ay, b]
    // 直接提取笛卡尔速度分量（状态4,5,6）
    return Eigen::Vector3d(predicted_state(4), predicted_state(5), predicted_state(6));
}
    ...
    try {
        ...
        if (config["predictor"]) {
            ...
            if (predictor["tracker"]) {
                ...
                config_.min_init_count = predictor["tracker"]["min_init_count"].as<int>(2);  // V3需要至少2个点
            }
            
            // 注意：V3版本不需要加载EKF噪声参数，因为它们是内置的
        }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        RCLCPP_INFO(rclcpp::get_logger("main"), 
                   "使用TrackQueueV3滤波器 - 笛卡尔坐标恒定加速度模型");
        RCLCPP_INFO(rclcpp::get_logger("main"), 
