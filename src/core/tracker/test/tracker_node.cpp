...
TrackerNode::TrackerNode() 
    ...
    coord_converter_ = solver_->getCoordConverter();
    ...
    if (!targets.empty()) {
        ...
        for (const auto& target : targets) {
            ...
            for (const Eigen::Vector4d& xyza : armor_list) {
                ...
                // // clang-format off
                // Eigen::Matrix3d R = utils::rotation_matrix_yxz(xyza[3], pitch, roll);

                // utils::logger()->debug("构建的旋转矩阵是");
                // for (int i = 0; i < 3; ++i) {
                ...
                auto image_points = yaw_optimizer_->reproject_armor_out(world_point, xyza[3], target.armor_type, target.name);
                // utils::logger()->debug(
            ...
            // Eigen::Vector4d& xyza_first = armor_list[0];

            // utils::logger()->debug(
            //     "当前的armor_list[0]的是角度是:{:.2f}",
            //     xyza_first[3]
            // );
            // 这里的armor_list[0]一般就是正对相机的那个
