...
bool Traditional_Detector::detect(Armor & armor, const cv::Mat & bgr_img)
{
  // 取得四个角点
...
bool Traditional_Detector::check_geometry(const Lightbar & lightbar) const
{
...
bool Traditional_Detector::check_geometry(const Armor & armor) const
{
...
bool Traditional_Detector::check_name(const Armor & armor) const
{
  ...
  if (armor.name == ArmorName::five) utils::logger()->debug("See pattern 5");

  return name_ok && confidence_ok;
}

bool Traditional_Detector::check_type(const Armor & armor) const
{
  ...
  // if (!name_ok) {
  //   utils::logger()->debug(
  //     "see strange armor: {} {}", ARMOR_TYPES[armor.type], armor_name_to_string(armor.name));
  //   save(armor);
  // }
...
cv::Point2f Traditional_Detector::get_center_norm(const cv::Mat & bgr_img, const cv::Point2f & center) const
{
  auto h = bgr_img.rows;
  auto w = bgr_img.cols;
...
void Traditional_Detector::save(const Armor & armor) const
{
...
// 公开的可视化接口，与神经网络检测器接口兼容
void Traditional_Detector::visualize_results(
  cv::Mat & canvas,
  const std::vector<Visualization> & armors,
  const cv::Point & center_point,
  int frame_index) const
{
  // 绘制帧号
  if (frame_index > 0) {
    utils::draw_text(canvas, fmt::format("[{}]", frame_index), {10, 30}, {255, 255, 255});
  }
  ...
  for (const auto & armor : armors) {
    ...
    // 绘制装甲板名称和类型
    std::string label = armor_name_to_string(armor.name) + " ," +
                       ARMOR_TYPES[armor.type];
    utils::draw_detection_label(canvas, label, label_pos, utils::colors::GREEN, 0.7, 2);
  }
...
void Traditional_Detector::show_result(
  ...
  const std::list<Armor> & armors, int frame_count) const
{
  ...
  for (const auto & lightbar : lightbars) {
    ...
    utils::draw_text(detection, info, lightbar.top, {0, 255, 255});
    utils::draw_points(detection, lightbar.points, {0, 255, 255}, 3);
  }

  for (const auto & armor : armors) {
    auto info = fmt::format(
      "{:.2f} {:.2f} {:.1f} {:.2f} {} {}", armor.ratio, armor.side_ratio,
      armor.rectangular_error * 57.3, armor.confidence, armor_name_to_string(armor.name),
      ARMOR_TYPES[armor.type]);
    utils::draw_points(detection, armor.points, {0, 255, 0});
    utils::draw_text(detection, info, armor.left.bottom, {0, 255, 0});
  }
...
void Traditional_Detector::lightbar_points_corrector(Lightbar & lightbar, const cv::Mat & gray_img) const
{
  // 配置参数
