...
void Solver::solve(std::optional<PowerRune> & ps) const
{
  ...
  p.ypd_in_world = utils::xyz2ypd(p.xyz_in_world);

  p.blade_xyz_in_world = R_gimbal2world_ * blade_xyz_in_gimbal;
  p.blade_ypd_in_world = utils::xyz2ypd(p.blade_xyz_in_world);

  p.ypr_in_world = utils::eulers(R_buff2world, 2, 1, 0);
}
...
{
  auto R_buff2world = utils::rotation_matrix_zyx(yaw, 0.0, row);
  // clang-format on
