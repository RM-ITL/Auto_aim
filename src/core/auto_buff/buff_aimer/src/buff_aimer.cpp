  ...
  double bullet_speed, bool to_now)
{
  ...
  auto detect_now_gap = utils::delta_time(now, timestamp);
  ...
  if (switch_fanblade_) {
    command.shoot = false;
    last_fire_t_ = now;
  } else if (!switch_fanblade_ && utils::delta_time(now, last_fire_t_) > fire_gap_time_) {
    command.shoot = true;
    last_fire_t_ = now;
  }
  ...
  if (get_send_angle(target, future, bullet_speed, to_now, yaw, pitch)) {
    ...
    if (plan.control) {
      if (first_in_aimer_) {
        ...
        plan.yaw_vel = utils::limit_rad(yaw - last_yaw_mpc) / (2 * dt);
        // plan.yaw_vel = utils::limit_min_max(plan.yaw_vel, -6.28, 6.28);
        plan.yaw_acc = (utils::limit_rad(yaw - gs.yaw) - utils::limit_rad(gs.yaw - last_yaw_mpc)) /
                       std::pow(dt, 2);
        // plan.yaw_acc = utils::limit_min_max(plan.yaw_acc, -50, 50);

        plan.pitch_vel = utils::limit_rad(-pitch + last_pitch_mpc) / (2 * dt);
        // plan.pitch_vel = utils::limit_min_max(plan.pitch_vel, -6.28, 6.28);
        plan.pitch_acc = (-pitch - gs.pitch - (gs.pitch + last_pitch_mpc)) / std::pow(dt, 2);
        // plan.pitch_acc = utils::limit_min_max(plan.pitch_acc, -100, 100);
      }
    }
  }
  ...
  const bool to_now, double & yaw, double & pitch)
{
  ...
  // 创建弹道对象
  utils::Trajectory trajectory0(bullet_speed, d, h);
  ...
  h = aim_in_world[2];
  utils::Trajectory trajectory1(bullet_speed, d, h);
  ...
  if (std::abs(time_error) > 0.01) {  // 如果时间误差过大，返回未命中结果
    utils::logger()->debug("[Aimer] Large time error: {:.3f}", time_error);
    return false;
  }
