...
public:
  ...
  ~SocketCAN()
  {
    ...
    if (read_thread_.joinable()) read_thread_.join();
    close();
    utils::logger()->info("SocketCAN destructed.");
  }
...
private:
  ...
  void open()
  {
    ...
    read_thread_ = std::thread([this]() {
      ...
      while (!quit_) {
        ...
        try {
          ...
          utils::logger()->warn("SocketCAN::read() failed: {}", e.what());
          ok_ = false;
          break;
        }
      }
    ...
    utils::logger()->info("SocketCAN opened.");
  }
  ...
  void try_open()
  {
    try {
      ...
      utils::logger()->warn("SocketCAN::open() failed: {}", e.what());
    }
  }
