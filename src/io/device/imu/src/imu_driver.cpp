  ...
  queue_(kQueueCapacity)
{
  ...
  utils::logger()->info("[DmImu] initialized");
}
...
DmImu::~DmImu()
{
  ...
  utils::logger()->info("[DmImu] destroyed");
}
...
void DmImu::init_imu_serial()
{
  try {
    ...
    switch (imu_seial_baud_) {
      ...
      default:
        utils::logger()->warn(
          "[DmImu] unsupported baud rate {} detected, fallback to 921600", imu_seial_baud_);
        imu_seial_baud_ = 921600;
        break;
    }
    ...
    sleep_for_post_config();
    utils::logger()->info("[DmImu] serial port {} opened", imu_serial_port_);
  } catch (const serial::IOException & e) {
    utils::logger()->error("[DmImu] serial IO exception: {}", e.what());
    throw;
  } catch (const std::exception & e) {
    utils::logger()->error("[DmImu] serial initialization failed: {}", e.what());
    throw;
  }
...
void DmImu::enter_setting_mode()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in enter_setting_mode: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::turn_on_accel()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in turn_on_accel: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::turn_on_gyro()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in turn_on_gyro: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::turn_off_euler()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in turn_off_euler: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::turn_on_quat()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in turn_on_quat: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::set_output_interval_ms(uint16_t interval_ms)
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in set_output_interval_ms: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::save_imu_para()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in save_imu_para: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::exit_setting_mode()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in exit_setting_mode: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::restart_imu()
{
  ...
  for (int i = 0; i < 5; ++i) {
    try {
      ...
      utils::logger()->error("[DmImu] write error in restart_imu: {}", e.what());
    }
    sleep_for_command();
  }
}
...
void DmImu::get_imu_data_thread()
{
  ...
  utils::logger()->info("[DmImu] data thread started");

  while (!stop_thread_) {
    if (!serial_.isOpen()) {
      utils::logger()->warn("[DmImu] serial port not open when reading");
    ...
    try {
      ...
      while (parsed_frame && pending_bytes_.size() >= 4) {
        ...
        if (pending_bytes_[0] != 0x55 || pending_bytes_[1] != 0xAA || pending_bytes_[2] != 0x01) {
          ...
          ++error_num;
          if (error_num > 1200) {
            utils::logger()->warn("[DmImu] header resync triggered");
            error_num = 0;
          }
          parsed_frame = true;
          continue;
        ...
        switch (reg_type) {
          ...
          default:
            ...
            ++error_num;
            if (error_num > 1200) {
              utils::logger()->warn("[DmImu] unknown register type {:02X}", reg_type);
              error_num = 0;
            }
            parsed_frame = true;
            continue;
        ...
        if (pending_bytes_[frame_size - 1] != 0x0A) {
          ...
          ++error_num;
          if (error_num > 1200) {
            utils::logger()->warn("[DmImu] invalid frame tail for reg {:02X}", reg_type);
            error_num = 0;
          }
          parsed_frame = true;
          continue;
        ...
        if (packet_count % 1000 == 0) {
          // utils::logger()->info(
          //   "[DmImu] processed {} packets, pushed {} quaternions", packet_count, pushed_count);
        }
      ...
      utils::logger()->error("[DmImu] serial read exception: {}", e.what());
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    } catch (const std::exception & e) {
      utils::logger()->error("[DmImu] unexpected read error: {}", e.what());
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
  }

  utils::logger()->info("[DmImu] data thread stopped");
}
