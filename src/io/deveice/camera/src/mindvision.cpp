...
{
  set_vid_pid(vid_pid);
  if (libusb_init(NULL)) utils::logger()->warn("Unable to init libusb!");
...
MindVision::~MindVision()
{
  ...
  if (capture_thread_.joinable()) capture_thread_.join();
  close();
  utils::logger()->info("Mindvision destructed.");
}
...
void MindVision::open()
{
  ...
  capture_thread_ = std::thread{[this] {
    ...
    while (!quit_) {
      ...
      if (status != CAMERA_STATUS_SUCCESS) {
        utils::logger()->warn("Camera dropped!");
        ok_ = false;
        break;
      }
  ...
  utils::logger()->info("Mindvision opened.");
}
...
void MindVision::set_vid_pid(const std::string & vid_pid)
{
  ...
  if (index == std::string::npos) {
    utils::logger()->warn("Invalid vid_pid: \"{}\"", vid_pid);
    return;
  }
...
void MindVision::reset_usb() const
{
  ...
  if (!handle) {
    utils::logger()->warn("Unable to open usb!");
    return;
  }

  if (libusb_reset_device(handle))
    utils::logger()->warn("Unable to reset usb!");
  else
    utils::logger()->info("Reset usb successfully :)");
