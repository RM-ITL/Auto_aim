...
{
  utils::logger()->info("[Cboard] Waiting for q...");
  queue_.pop(data_ahead_);
  queue_.pop(data_behind_);
  utils::logger()->info("[Cboard] Opened.");
}
...
void CBoard::callback(const can_frame & frame)
{
  ...
  if (frame.can_id == quaternion_canid_) {
    ...
    if (std::abs(x * x + y * y + z * z + w * w - 1) > 1e-2) {
      utils::logger()->warn("Invalid q: {} {} {} {}", w, x, y, z);
      return;
    }
    ...
    if (bullet_speed > 0 && utils::delta_time(now, last_log_time) >= 1.0) {
      utils::logger()->info(
...
std::string CBoard::read_yaml(const std::string & config_path)
{
  ...
  quaternion_canid_ = static_cast<canid_t>(utils::read<int>(yaml, "quaternion_canid"));
  bullet_speed_canid_ = static_cast<canid_t>(utils::read<int>(yaml, "bullet_speed_canid"));
  send_canid_ = static_cast<canid_t>(utils::read<int>(yaml, "send_canid"));
