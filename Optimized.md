...
以上为早期尝试结合天大和上交开源做的工作，因为比较零散加上结合两个包做迁移和修改十分的难受，再加上文件结构修改的乱七八糟，没有按照
良好的设计思维和格式去修改，导致做出来的效果实际上很有限，没有反小陀螺，平移和小陀螺估计器解耦，常常就是拟合估计效果很差，再加上电控
接口部分有屎，看不出什么效果，所以废弃了，下面则是做的对同济的开源和修改，严格按照封装解耦良好的格式去修改和迁移设计，看起来没那么屎，
同时参考使用对每个模块进行严格的封装和明确责任边界，不再走多节点，而是走多动态库，加一个核心节点的设计思路，即规避了多模块多节点造成的通讯
问题，又保留了ROS，可以使用ROS工具
...
1. 明确了新赛季新规则变化，在自瞄方面需要修改的还有不少
# 前哨站的估计修改，现在改为了两个固定，一个旋转，呈现120度分布，且三个有上下高低差

# 大符需要修改，之前是2.5s内激活其中随机一个灯臂，现在则是3.5s内有两个随机灯臂，实际上是2.5s内需要打击第一个，然后剩下1s内打击第二个，但是1s内连续打击难度极高，所以就可以看作是3.5s内打击两个中一个就行
...
[深入解析]：
针对装甲板ID的匹配，在设置上是0号(短轴低板)——正对, 1号(长轴高板)——左面, 2号(短轴低板)——背面, 3号(长轴高板)——右面
初始化阶段，在实际的解算的时候选择观测到的优先级最高的装甲板，进行solver的解算填充对应的Armor_pose，然后送入构造函数中进行Target的对象初始化，
在初始化阶段的时候，默认该观测到的装甲板为短轴低板0号——装甲板，使用的角度也是解算出来该装甲板的angle，且此时的状态变量中的l与都为0,默认此时的
4个装甲板是处于同一个圆周，同一个高度
...
当前就是：
  | 装甲板ID | 轴长       | 高度       | 说明           |
  |-------|----------|----------|--------------------|
  | ID 0  | 短轴 (r)   | 低板 (z)   | use_l_h = false |
  | ID 1  | 长轴 (r+l) | 高板 (z+h) | use_l_h = true  |
  | ID 2  | 短轴 (r)   | 低板 (z)   | use_l_h = false |
  | ID 3  | 长轴 (r+l) | 高板 (z+h) | use_l_h = true  |


如果想要修改为短轴高板，长轴低板，只需要修改l和h使用的判断就可以：
       365    
       366    // 计算出装甲板中心的坐标（考虑长短轴）
       367 +  // 修改后的配置：ID 0,2为短轴高板，ID 1,3为长轴低板
       368    Eigen::Vector3d Target::h_armor_xyz(const Eigen::VectorXd & x, int id) const
       369    {
       370      auto angle = utils::limit_rad(x[6] + id * 2 * CV_PI / armor_num_);
       371    
       372 +    // 反转逻辑：ID 0,2 使用 l 和 h，ID 1,3 不使用
       373 +    auto use_l = (armor_num_ == 4) && (id == 0 || id == 2);  // 短轴使用l
       374 +    auto use_h = (armor_num_ == 4) && (id == 0 || id == 2);  // 短轴使用h
       375 +  
       376 +    auto r = (use_l) ? x[8] + x[9] : x[8];
       377      auto armor_x = x[0] - r * std::cos(angle);
       378      auto armor_y = x[2] - r * std::sin(angle);
       379 +    auto armor_z = (use_h) ? x[4] + x[10] : x[4];
       380    
       381      return Eigen::Vector3d(armor_x, armor_y, armor_z);
       382    }


进入更新阶段之后，获取预测的四个装甲板的位置，然后选择最近的三个，忽略背后板，进行装甲板ID的纠正或者更新，通过装甲板的朝向角和位置角来进行更新，确保其得到
与真实情况匹配的ID，例如短轴低板就是0或者2,长轴高板就是1或者3

  匹配逻辑：
  1. 预测：基于当前状态 x[6], x[8], x[9], x[10] 计算4个装甲板位置
  2. 距离筛选：只考虑距离最近的3个（排除背面板）
  3. 角度匹配：
    - 装甲板姿态角 world_orientation.yaw
    - 装甲板位置角 world_spherical.yaw
    - 两者与预测值的差异最小的即为匹配ID
...
auto angle = utils::limit_rad(x[6] + id * 2 * CV_PI / armor_num_);
  | 装甲板ID | 角度偏移         | 说明        |
  |-------|--------------|-----------------|
  | ID 0  | angle + 0°   | 初始观测装甲板方向 |
  | ID 1  | angle + 120° | 逆时针旋转120° |
  | ID 2  | angle + 240° | 逆时针旋转240° |


同时针对目标的追踪状态的处理，
  lost ──发现目标──> detecting ──连续检测──> tracking
                                                │
                                            丢失目标
                                                ↓
                                           temp_lost
                                      ...
                                      找回目标      超时
                                         ↓           ↓
                                     tracking      lost
只有在目标完全的lost的时候才会被销毁对应的Target对象，否则将继续维护其对象

[前哨站估计修改]：
因为原有的估计器设置是固定的，对于ID的设置也是固定的，为了适应新的机构，上中下三个高度，这里设置：
  | ID  | 位置  | 高度    | dz/dh |
  |-----|-----|-------|-------|
  | 0   | 上部  | z + h | +1    |
  | 1   | 中部  | z     | 0     |
...
这里的状态变量x[10]对于前哨站的定义为单层高度偏差，原本对于常规步兵为高低板的差高，现在重新定义了一下，之后因为这里可能会出现同时观测到中部和上部，或者先单独观测到中部或者上部
所以初始ID不会是肯定正确的，所以这里在ID匹配的时候除了原有的角度，还引入了高度的匹配，两个联合去匹配确保收敛

然后设置前哨站的h先验为0.135，小协方差，同时设置好前哨站的雅可比矩阵的修改：
...
1. 
[自瞄yaw漂移问题]：
在上位机方面，解算solver模块使用以下的策略来抵抗零漂和噪声

  1. IMU提供的是绝对姿态四元数，不是角速度积分，所以零漂极小
  2. PnP每帧都直接从图像测量，提供绝对位置观测，不会累积误差
  3. 静止场景下目标不动，过程噪声Q只影响预测，但观测R一直在修正
  4. 自适应R矩阵在静止场景下观测质量稳定，EKF快速收敛到真实值
  5. 较大的过程噪声允许EKF相信观测而非预测，快速纠正任何微小偏差
  ...
  │   抵抗IMU零漂的五层防护机制                │
  ├─────────────────────────────────────────────┤
  │ 1. IMU使用绝对姿态，不积分 ← 根本性避免   │
  │ 2. PnP提供绝对测量，独立观测 ← 不传播误差 │
  │ 3. 自适应观测噪声R ← 降低坏观测影响        
...
在今天实际的测试时候发现，在不与电控通讯，纯让电控固定云台去测试，发现瞄准的装甲板依然在图像坐标系中的中心点像素坐标依然在漂移，排除了上位机的问题，但是问题依旧存在，就是在
gimbal_yaw和上位机发送的yaw上依然是误差很小，在像素上已经跑飞20像素了，但是在角度上看起来就是从0.004到0.007的弧度误差

经过测试发现，是下位机使用IMU积分出来的yaw来进行控制的云台角度，这就导致了像素的漂移，同时可能因为上下位机的IMU的零漂的速率是不同的，导致了
这个上下位机之间的delta_yaw一直都有且在缓慢增大，由0.004一直累积增长到0.007弧度
...
在IMU上电的时刻，此时定义为世界坐标系下的零点，此时的ypr都是0,然后实际上IMU内部的驱动写的是
输出相对于上电那个零点时刻，此时IMU所处姿态得到的绝对四元数姿态变换，在solver模块内部变换之后，
实时的更新变换坐标系，经过解算之后填充得到Armor_pose，之后在估计器构建Target对象之后，将Target对象
传给planner规划器，规划器内部拿到target对象之后，选择距离最近的那个装甲板，得到其xyz坐标，加上yaw_offest
得到yaw0，这里的yaw0就是目标在世界坐标系中的绝对方位角

  IMU上电 (t=0)
    ↓
  IMU固件定义零点：当前yaw=0, pitch=0, roll=0
    ↓
  运行时每一帧：
    orientation = dm_imu_->imu_at(timestamp)  // 相对于上电时刻的姿态
    solver_->updateIMU(orientation, ...)
    ↓
    R_gimbal_to_world = R_gimbal_to_imu^T * R_imu_current * R_gimbal_to_imu
    ↓
  目标xyz_world = R_gimbal_to_world * ...
    ↓
  yaw0 = atan2(y_world, x_world)  // 相对于上电时刻定义的世界坐标系x轴

在planner中规划输出的实际计算的是相对角度，最后在实际的赋值的时候，把yaw0加回去就得到了对应的绝对角度
  ...
  把MPC求解得到的相对角度加回yaw0，就得到了世界坐标系下的绝对角度。

[变换链路]：
  ┌──────────────┬─────────────────┬──────────────────┐
  │   模块       │   数据          │   坐标系         │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Detector     │ pixels          │ 图像坐标系       │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Solver(PnP)  │ xyz_camera      │ 相机坐标系       │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Solver(变换) │ xyz_gimbal      │ 云台坐标系       │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Solver(变换) │ xyz_world       │ 世界坐标系       │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Tracker      │ Target.ekf_x()  │ 世界坐标系         │
  │              │ armor_xyza_list │                  │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Planner(aim) │ xyz, yaw0       │ 世界坐标系         │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Planner(MPC) │ traj相对角度    │ 相对于yaw0         │
  ├──────────────┼─────────────────┼──────────────────┤
  │ Planner(输出)│ plan.yaw        │ 世界坐标系(绝对)    │
  └──────────────┴─────────────────┴──────────────────┘

在planner内部规划的时候，其中输出的plan.yaw是目标所处在世界坐标系中的绝对方位角加上其轨迹变化中移动的角度，MPC输出的是相对yaw0的相对角度：

  yaw0 = 当前帧目标在世界坐标系的方位角
       = atan2(target.y_world, target.x_world)

  traj中的相对角度 = 未来时刻目标的角度 - yaw0
                  = 相对于"当前帧目标方向"的偏移

  物理含义

  时刻t₀:  目标在yaw0 = 0.5 rad的方向
  时刻t₁:  目标移动到yaw = 0.55 rad
           相对角度 = 0.55 - 0.5 = 0.05 rad
                    ↑
              表示目标自己移动了0.05 rad

  这个相对角度描述的是：目标自己的运动导致的角度变化！
...
1.[传统识别的流程]：
  ...
  ┌─────────────────────────────────────────────┐
  │            灯条提取阶段                       │
  │  轮廓检测 → 旋转矩形拟合 → 几何过滤 → 颜色识别 │
  └─────────────────────────────────────────────┘
        ↓
  ┌─────────────────────────────────────────────┐
  │            装甲板匹配阶段                     │
  │  灯条配对 → 几何验证 → 分类器识别 → 类型判断   │
  └─────────────────────────────────────────────┘
        ↓
  ┌─────────────────────────────────────────────┐
  │            后处理阶段                        │
...
1. 阈值化的时候选择是固定阈值：
# 这里使用的threshold是150,可以考虑优化，使用OTSU或者直方图分析
...
2.然后灯条轮廓的提取和对象的构造：
# 这里是对找到的轮廓进行最小外接矩形拟合，然后对其构造灯条LightBar对象，再排序一下

  Lightbar数据结构 (armor.hpp:78-101):
  - angle: 灯条倾斜角度
  - angle_error: 与垂直方向的角度偏差
  - ratio: 长/宽 比值
  - length, width: 尺寸
  - top, bottom, center: 关键点坐标

对每个轮廓：
1. 使用 cv::minAreaRect() 获取最小外接矩形
2. 构造 Lightbar 对象
3. 几何约束检查 (check_geometry):
  - 角度误差 < 45°（灯条接近竖直）
  - 宽高比在 1.5 ~ 20 之间
  - 最小长度 > 8 像素
4. 颜色判断: 统计轮廓点的红蓝通道值，蓝色多则为蓝方，红色多则为红方

# 这里的颜色判断有些不妥，待修改

3. 装甲板的匹配：
# 对灯条进行匹配进行判断，是否可以构成装甲板：检查其配对之后的装甲板的宽高比，左右灯条比，矩形差，然后提取ROI之后分类，进行其type和name的提取：
  灯条按x坐标从左到右排序后，进行两两配对：
  1. 颜色必须相同
  2. 几何约束检查:
    - 宽高比在 1 ~ 5 之间
    - 左右灯条长度比 < 1.5
...
# PCA灯条校正：
  ...
  一、整体流程图
    ...
    └─────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
    ┌─────────────────────────────────────────────────────────────────┐
    │  输出: 更新 lightbar.top 和 lightbar.bottom                       │
    └─────────────────────────────────────────────────────────────────┘

    ---
    二、分步详解

    Step 1: ROI 提取与预处理 (行 428-442)

    // 扩展并裁剪ROI
    cv::Rect roi_box = lightbar.rotated_rect.boundingRect();
    roi_box.x -= roi_box.width * ROI_SCALE;      // 向左扩展 7%
    roi_box.y -= roi_box.height * ROI_SCALE;     // 向上扩展 7%
    roi_box.width += 2 * roi_box.width * ROI_SCALE;   // 宽度扩展 14%
    roi_box.height += 2 * roi_box.height * ROI_SCALE; // 高度扩展 14%
    ...
    ---
    Step 2: 计算灯条质心 (行 444-447)

    const cv::Moments moments = cv::moments(roi);
    const cv::Point2f centroid(
      moments.m10 / moments.m00 + roi_box.x,   // X 坐标 (转换回原图坐标系)
      moments.m01 / moments.m00 + roi_box.y);  // Y 坐标
    ...
    作用：质心作为后续搜索的起点，比几何中心更准确地反映灯条的"重心"位置。

    ---
    Step 3: PCA 计算主轴方向 (行 449-464)

    // 生成点云：收集所有亮度 > 0.001 的像素坐标
    std::vector<cv::Point2f> points;
    for (int i = 0; i < roi.rows; ++i) {
      for (int j = 0; j < roi.cols; ++j) {
        if (roi.at<float>(i, j) > 1e-3) {
    ...
    axis /= cv::norm(axis);    // 归一化为单位向量
    if (axis.y > 0) axis = -axis;  // 统一方向：确保 axis 指向上方
    ...
    为什么用 PCA：
    - minAreaRect 只考虑轮廓边界
    - PCA 考虑所有亮像素的分布，对噪声和边缘不规则更鲁棒

    ---
    Step 4: 沿主轴搜索亮度跳变点 (行 466-522)

    这是核心部分，使用 lambda 函数 find_corner 实现：

    const auto find_corner = [&](int direction) -> cv::Point2f {
      // direction = 1 表示向上搜索（找 top）
      // direction = -1 表示向下搜索（找 bottom）

      const float dx = axis.x * direction;
      const float dy = axis.y * direction;
      const float search_length = lightbar.length * (SEARCH_END -
    SEARCH_START);
      // 搜索范围：从灯条长度的 40% 位置开始，到 60% 位置结束
    ...
    for (float step = 0; step < search_length; ++step) {
      const cv::Point2f cur_point(start_point.x + dx * step, 
                                  start_point.y + dy * step);

      // 计算相邻两点的亮度差
      const auto prev_val = gray_img.at<uchar>(...);  // 前一个点亮度
      const auto cur_val = gray_img.at<uchar>(...);   // 当前点亮度
      const float diff = prev_val - cur_val;  // 亮度下降值

      // 找最大亮度跳变位置
      if (diff > max_diff && prev_val > mean_val) {
        max_diff = diff;
        corner = cur_point - cv::Point2f(dx, dy);  // 跳变点的前一位置
        found = true;
      }
    }
    ...
    return candidates.empty()
            ? cv::Point2f(-1, -1)
            : std::accumulate(candidates.begin(), candidates.end(),
    cv::Point2f(0, 0)) /
    ...
    | 参数             | 值    | 含义                   |
    |----------------|------|----------------------|
    | MAX_BRIGHTNESS | 25   | 归一化后的最大亮度值           |
    | ROI_SCALE      | 0.07 | ROI 边缘扩展比例 (7%)      |
    | SEARCH_START   | 0.4  | 从质心向外搜索起点（灯条长度的40%处） |
    | SEARCH_END     | 0.6  | 搜索终点（灯条长度的60%处）      |
    ...
    从 40% 到 60% 搜索，是因为角点预期在灯条的两端，从中间向外搜索可以避免灯条
    内部亮度不均匀的干扰。
    ...
    1. 精度高: PCA + 亮度梯度比 minAreaRect 更准确
    2. 鲁棒性强: 多线采样 + 均值融合抗噪声
    3. 自适应: 基于实际亮度分布而非几何边界

    缺点

    1. 计算量较大: 需要遍历 ROI 所有像素 + PCA 分解
    2. 依赖参数: 搜索范围、阈值等需要调优
    3. 当前代码中被关闭: 在 detector.cpp:179 注释掉了调用


# 可修改测试;

1. armor_type:
直接根据装甲板识别出的name来进行判断其type,除了英雄和基地給big，其他都直接给small

2. 阈值化：
当前为给定的固定阈值操作，阈值系数通过配置文件加载，当前是140,思考采用OTSU或者直方图分析阈值来测试看看效果

3. 灰度化与颜色的检测：

当前的颜色判断是统计红蓝的通道值，如果蓝>红，则标记为蓝色，否则是红色，虽然上场以后很少有白灯的干扰，但是线下测试的时候还是进行一下修改看看，准备的方案之一是HSV检测来判断具体的颜色

灰度化当前采用标准的灰度化，思考可以采用通道差分，但是通道差分需要指定颜色，所以有点背道而驰，也可以采用HSV色相的筛选提取，但是对于红蓝效果可能没那么好
...
与新前哨站估计器大战
经历了一顿战斗，终于是有明显的进步的效果，现在进入参数调节的阶段：
一共有以下参数可调节：

  | 参数  | 位置                  | 当前值 | 建议测试范围 |
  |-------|-----------------------|--------|--------------|
  | v_h   | outpost_target.cpp:85 | 0.001  | 0.001 ~ 0.005  | 
  # h1和h2的过程噪声，对比三个参数，原本的0.001，以及调节了两次的0.005和0.01，目前来看0.005效果最好，明显在旋转过程中也能正确的估计
  | vx    | tracker.cpp:288       | 64     | 4 ~ 16       |
  | vy    | tracker.cpp:288       | 64     | 4 ~ 16       |
  | vz    | tracker.cpp:288       | 81     | 1 ~ 4        |
  # center_x,y,z的初始协方差，都有点高，可以调低一些，因为前哨站目标的center_x,y,z基本不动
  | P0_h1 | tracker.cpp:288       | 0.1    | 0.3 ~ 0.5    |
  | P0_h2 | tracker.cpp:288       | 0.1    | 0.3 ~ 0.5    |


# 第一次测试：v_h = 0.01
# 第二次测试：v_h = 0.005
# 第三次测试：v_h = 0.001
# 第四次测试：v_h = 0.005 + vx,vy,vz = 16
# 第五次测试: v_h = 0.005 + h1,h2 = 0.2
# 第六次测试: v_h = 0.005 + h1,h2 = 0.05，h1和h2调低是负作用！！
# 第七次测试: v_h = 0.005 + h1,h2 = 0.3
# 传统不如神经网络
